...
{
	SDLOG_PRINTF("END", SD_LOG_LEVEL_INFO, "Please input exit to exit.");
}
#endif
...
int main(int argc, char *argv[])
{
	...
	tRLimit.rlim_max = RLIM_INFINITY;
	tRLimit.rlim_cur = RLIM_INFINITY;
	if (setrlimit(RLIMIT_CORE, &tRLimit) != 0)
	{
		SDLOG_PRINTF("main", SD_LOG_LEVEL_ERROR, "setrlimit failed, errno:%d", errno);
	}
